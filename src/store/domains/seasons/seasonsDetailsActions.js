...
const detailsForSeason = (season) => {
    return async(dispatch,getState) => {        
        ...
        if ( res.error ) 
            return dispatch({type:'CURSOR_ERROR',message:`Ошибка получения данных ${res.error}`,info:{actionName:'detailsForSeason',reducersName:'seasonDetailsReducers',cursorName:'seasonDetails'}});
        
        const details =[{
            title:season.season
            ,data:res.items
        }]
        dispatch({type:'SEASON_DETAILS_FETCHED',details,cursor:{...res.cursor,page:0},seasonDetails,season});
    }
}
...
const fetch = () => {
    return async(dispatch,getState) => {
        ...
        const res = await seasonDetails.move(cursor.page);
        if ( res.error ) 
            return dispatch({type:'CURSOR_ERROR',message:`Ошибка получения данных ${res.error}`,info:{actionName:'fetch',reducersName:'seasonDetailsReducers',cursorName:'seasonDetails'}});
        
        const details =[{
            title:season.season
            ,data:res.items
        }]
        dispatch({type:'SEASON_DETAILS_FETCHED',details,cursor:res.cursor,seasonDetails,season});
    }
}
